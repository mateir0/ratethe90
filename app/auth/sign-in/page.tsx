    ...
    <AppShell>
      <main className="space-y-4">
        ...
        <form onSubmit={onSubmit} className="space-y-3">
          ...
          <button className="w-full rounded bg-[#22C55E] py-3 font-semibold text-black">Sign in</button>
        </form>
        {message && <p role="status" className="text-sm text-[#A3E635]">{message}</p>}
        <Link href="/auth/sign-up" className="text-sm text-[#94A3B8] underline">Need an account?</Link>
      </main>
    </AppShell>
  );
}
